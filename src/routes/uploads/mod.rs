...
pub async fn uploads_file(content_type: &ContentType, data: Data<'_>) -> Value {
    ...
    let options: MultipartFormDataOptions = MultipartFormDataOptions {
        allowed_fields: iter::repeat(
            MultipartFormDataField::file("file").content_type(Some(mime::IMAGE_STAR)),
        )
    ...
    for _file in files {
        ...
        let file_name = String::from(_file.file_name.as_ref().unwrap());
